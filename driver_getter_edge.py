...
def get_edge_user_data_dir():  
    ...
    if system == "Windows":  
        ...
        return f"/Users/{username}/Library/Application Support/Microsoft Edge"  
    elif system == "Linux":  
        return f"/home/{username}/.config/microsoft-edge"  
    else:  
        return None  


def create_normal_edge_driver(temp_user_data = r'C:\Users\25131\AppData\Local\Temp\edge_user_data_1feee482',
    ...
    if world_size:
        options.add_argument(f'--window-size={world_size[0]},{world_size[1]}')  
    else:
    ...
    options.add_argument('--load-extension=')  # 设置空的扩展加载路径
    ...
    try:
        ...
        raise RuntimeError(f"无法启动Edge浏览器: {str(e)}") from e
        ...
        for proc in psutil.process_iter(['pid', 'name']):
            ...
            try:
                if 'msedge' in proc.info['name'].lower():
                    # 'msedge' in ...：检查进程名称中是否包含 "msedge"（Microsoft Edge 浏览器的主进程名称）
                    proc.terminate()
                    proc.wait(timeout=5)
